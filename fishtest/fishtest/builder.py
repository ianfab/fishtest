    ...
    if len(target["gcc_alias"]) > 0:
        with open("tmp", "w") as out:
            with open("Makefile") as f:
                new_makefile = f.read()
                new_makefile = new_makefile.replace(
                    "CXX=g++", "CXX=" + target["gcc_alias"]
                )
                new_makefile = new_makefile.replace(
                    "$(EXTRALDFLAGS)",
                    "$(EXTRALDFLAGS) -static-libstdc++ -static-libgcc -static",
                )
