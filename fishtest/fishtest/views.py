...
def regression_request_isvalid(request):
    return ("type" in request.GET) and (
        request.GET["type"] == "fishtest" or request.GET["type"] == "jl"
    )


@view_config(
    route_name="regression_data",
    renderer="regression_data.mak",
...
@view_config(
    route_name="regression_data_save", request_method="POST", permission="modify_stats"
)
...
@view_config(
    route_name="regression_data_delete",
    request_method="POST",
    permission="modify_stats",
)
...
def tests_view(request):
    ...
    for name in [
        ...
        if name == "spsa" and value != "-":
            params = [
                "param: %s, best: %.2f, start: %.2f, min: %.2f, max: %.2f, c %f, a %f"
                % (
                    p["name"],
                    p["theta"],
                    p["start"],
            ...
            value = (
                "Iter: %d, A: %d, alpha %f, gamma %f, clipping %s, rounding %s\n%s"
                % (
                    value["iter"],
                    value["A"],
                    value["alpha"],
...
def tests(request):
    ...
    runs["pending"].sort(
        key=lambda run: (run["args"]["priority"], run["args"]["internal_priority"])
    )
    runs["active"].sort(
        reverse=True,
        key=lambda run: (
    ...
    def remaining_hours(run):
        ...
        return (
            game_secs * remaining_games * int(run["args"].get("threads", 1)) / (60 * 60)
        )
