...
class RunDb:
    ...
    def new_run(
        ...
        def get_approval(sha):
            q = {
                "$or": [{"args.resolved_base": sha}, {"args.resolved_new": sha}],
                "approved": True,
            }
    ...
    def get_finished_runs(
        self, skip=0, limit=0, username="", variant="", success_only=False
    ):
        ...
        if limit != 0 and len(result[0]) != limit:
            c = self.old_runs.find(
                q, skip=max(0, skip - c.count()), limit=limit - len(result[0])
            )
            result[0] += list(c)
            result[1] += c.count()
        else:
    ...
    def request_task(self, worker_info):
        ...
        existing_run = self.runs.find_one(
            {"tasks": {"$elemMatch": {"active": True, "worker_info": worker_info}}}
        )
        ...
        connections = sum(
            [int(m.get("remote_addr", "") == remote_addr) for m in machines]
        )
        ...
        q = {
            ...
            "query": {
                "$and": [
                    {"tasks": {"$elemMatch": {"active": False, "pending": True}}},
            ...
            "sort": [
                ("args.priority", DESCENDING),
                ("args.internal_priority", DESCENDING),
                ("_id", ASCENDING),
            ],
            "update": {
                "$set": {
        ...
        if run["args"]["throughput"] != None and run["args"]["throughput"] != 0:
            run["args"]["internal_priority"] = (
                -time.mktime(run["start_time"].timetuple())
                - task_id
                * 3600
                * self.chunk_size
                * run["args"]["threads"]
    ...
    def spsa_param_clip_round(self, param, increment, clipping, rounding):
        ...
        if rounding == "randomized":
            fl = math.floor(
                value
            )  # greatest integer <= value, thus works for negative.
    ...
    def request_spsa(self, run_id, task_id):
        ...
        iter_local = (
            spsa["iter"] + 1
        )  # assume at least one completed, and avoid division by zero
